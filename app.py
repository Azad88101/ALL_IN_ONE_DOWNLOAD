...
DOWNLOAD_FOLDER = os.path.join(os.path.expanduser("~"), "Downloads/website")
...
def generate_barcode():
    try:
        ...
        barcode_format = request.form.get('format', 'ean13')  # Default to 'ean13'

        # Log inputs for debugging
        print(f"Generating barcode. Format: {barcode_format}, Code: {code}")

        # Check if the barcode format is valid
        try:
            BARCODE_CLASS = barcode.get_barcode_class(barcode_format)
        except barcode.errors.BarcodeNotFoundError:
            return jsonify({'error': f'Invalid barcode format: {barcode_format}'}), 400

        # Validate the code for EAN-13 format (must be 13 digits)
        if barcode_format == 'ean13' and not (code.isdigit() and len(code) == 13):
            return jsonify({'error': 'EAN-13 barcode requires a 13-digit numeric code.'}), 400
        ...
        # Return the generated barcode image as a PNG file
